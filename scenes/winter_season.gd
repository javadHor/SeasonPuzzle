...
func _on_exit_door_body_entered(body):
	if body.name == "Player":
		if item_collected:
			print("شما از آخرین فصل خارج شدید. بازی به پایان رسید!")
			
			# از GameManager بخواه Win Screen رو نشون بده
			if game_manager:
				game_manager.show_win_screen()
			else:
				print("خطا: GameManager پیدا نشد!")
		else:
			print("برای خروج باید اول آیتم فصل زمستان رو جمع کنی!")
